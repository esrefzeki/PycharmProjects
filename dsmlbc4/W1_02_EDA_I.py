...
np.random.randint(0, 10, size=10) # np.random.randint(Bir sayı ile, Başka bir sayı arasında, Belirli bir sayı adedince)
np.random.normal(10, 4, (3, 4)) # np.random.normal(Dağılımın ortalama merkezi, Standart sapması, (kaça kaçlık bir matris olacağı))
np.random.randint(0, 10, (3, 3)) #np.random.randint ( Bir sayı ile, Başka bir sayı arasında, (kaça kaçlık matris ile oluşsun))
...
np.arange(1, 10) # Belirli sayılar arasında tek boyutlu bir liste oluşturur.
np.arange(1, 10).reshape((3, 3)) # Belirli sayılar arasında verilen değerler boyutunda matris oluşturur.
...
a = np.random.randint(10, size=10)
a[0] # a'ya atanan liste içi değerlerden ilk indexi getirir.
a[-2] # a'ya atanan liste içi değerlerden sondan ikinciyi getirir. Eksi değer sondan değer getirir.
a[0] = 999 # a'nın liste içi ilk değerine başka bir atama yapılabilir.
a
a[0:4]
...
m[0, 0]
m[1, 1]
m[2, 3] # [Satır (Row), Sütun(axis)] > Değeri verilen satırdan, değeri verilen sütun içerisindeki indexi getirir.
m[2, 4] # 2. satırdan, 4'üncü sütunu getir.
m[2, 3] = 9999
