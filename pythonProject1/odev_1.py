...
df.columns = [("NUM_"+sutun).upper() if df[sutun].dtype in ['int','float'] else sutun.upper() for sutun in df.columns]
...
df.columns = [(sutuns + "_FLAG").upper() if "no" not in sutuns else sutuns.upper() for sutuns in df.columns]
...
df = sns.load_dataset("car_crashes")
og_list = ["abbrev", "no_previous"]
new_sutunz = [sutunz for sutunz in df.columns if sutunz not in og_list]
new_df = df[new_sutunz]
new_df.head()
